...
def compare_by_return_time(a:Process,b:Process):
...
def merge(arr, l, m, r):
	...
	while i < n1 and j < n2:
		if L[i].arrive_time <= R[j].arrive_time:
			arr[k] = L[i]
			i += 1
		else:
			arr[k] = R[j]
...
def mergeSort_arrive_time(arr, l, r):
	if l < r:
		# Same as (l+r)//2, but avoids overflow for
		# large l and h
		m = l+(r-l)//2
		# Sort first and second halves
		mergeSort_arrive_time(arr, l, m)
		mergeSort_arrive_time(arr, m+1, r)
		merge(arr, l, m, r)
